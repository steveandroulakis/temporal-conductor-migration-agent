...
# Check Temporal CLI installed
echo "Checking Temporal CLI..."
command -v temporal >/dev/null 2>&1 || {
    echo "⚠️  Warning: Temporal CLI not found"
    echo "   Install: brew install temporal (macOS)"
    echo "   Or download from: https://temporal.io/download"
    echo ""
    echo "   You can still proceed, but you'll need Temporal CLI to run the workflow"
}
...
uv run mypy schema_approval_temporal --strict --ignore-missing-imports || {
...
echo ""
echo "5. Interact with running workflow:"
echo "   uv run interact query <workflow-id> get_status"
echo "   uv run interact update <workflow-id> submit_review1_approval '{...}'"
echo ""
